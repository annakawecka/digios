...
void fitting_170() {
	...
	for (int j = 0; j < nPeaks; ++j) {
            TF1 *peakFunc = new TF1(("peakFunc_" + std::to_string(j + 1)).c_str(), 
                                    "gaus(0)", hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax());
            peakFunc->SetParameter(0, fitFunc->GetParameter(3 * j + 2)); // Amplitude
            peakFunc->SetParameter(1, fitFunc->GetParameter(3 * j + 3)); // Mean (position)
            peakFunc->SetParameter(2, fitFunc->GetParameter(3 * j + 4)); // Sigma

	    peakFunc->SetNpx(1000);

            peakFunc->SetLineColor(4);
            peakFunc->SetLineStyle(1);
            peakFunc->Draw("SAME");
        }
	...
	std::ofstream outFile("17O_analysis/fit_parameters_17O.txt", std::ios::app);
	if (outFile.is_open()) {
    ...
    std::vector<std::vector<double>> peakPositions2 = {
        {0.15},
        {0.15, 1.982 + 0.15, 3.44},
        {1.982 + 0.15, 3.68, 3.92 + 0.15},
        {1.982 + 0.15, 3.68, 3.92 + 0.15, 5.255 + 0.15},
        {1.982 + 0.15, 3.68, 3.92 + 0.15, 5.255 + 0.15, 6.4, 7.20},
        {1.982 + 0.15, 3.68, 3.92 + 0.15, 5.255 + 0.15, 6.4, 7.20}
    };
    ...
    for (size_t i = 0; i < histNames.size(); ++i) {
        ...
        int nPeaks = peakPositions2[i].size();
        std::string funcExpr = "pol1"; // First-degree polynomial
        for (int j = 0; j < nPeaks; ++j) {
            funcExpr += "+gaus(" + std::to_string(3 * j + 2) + ")"; // Gaussians
        }
	...
	fitFunc->SetParName(0, "Background_Intercept");
        ...
        for (int j = 0; j < nPeaks; ++j) {
            fitFunc->SetParameter(3 * j + 2, hist->GetMaximum() * 0.5); // Amplitude
            fitFunc->SetParameter(3 * j + 3, peakPositions2[i][j]);     // Mean
            fitFunc->SetParameter(3 * j + 4, sigma);                   // Sigma

            fitFunc->SetParLimits(3 * j + 3, peakPositions2[i][j] - 0.1, peakPositions2[i][j] + 0.1);
            fitFunc->SetParLimits(3 * j + 4, 0.04, sigma);
	...
	stats->AddText(Form("Background Intercept: %.3f", fitFunc->GetParameter(0)));
        ...
        canvas->SaveAs((histNames[i] + "_fitting_mergedPeaks.png").c_str());

	std::ofstream outFile("17O_analysis/fit_parameters_17O_mergedPeaks.txt", std::ios::app);
	if (outFile.is_open()) {
	  outFile << "Histogram: " << histNames[i] << std::endl;
	  outFile << "Background Intercept: " << fitFunc->GetParameter(0) << std::endl;
	  outFile << "Background Slope: " << fitFunc->GetParameter(1) << std::endl;
