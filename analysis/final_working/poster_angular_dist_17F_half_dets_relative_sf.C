...
void poster_angular_dist_17F_half_dets_relative_sf() {
  ...
  fitPairs =  {
    {{25, 26, 27, 28}, {0.306, 0.102, 0.96, 0.89}}, // 0.937 & 1.041 & 1.121
    {{2, 3}, {0.17, 0.51}}, // 4.694, 1s1/2 and 0d5/2 SF ratio from 17O(3He,d)18F
    {{5, 6}, {0.21, 0.62}}, // 3.061, 1s1/2 and 0d5/2 SF ratio from 17O(3He,d)18F
    {{29, 30}, {1.04, 0.18}}, // 4.652 & 4.753, SF ratio from 17O(3He,d)18F
    {{21}, {1.0}}, // 4.360
  ...
  for (size_t mappingIndex = 0; mappingIndex < fitMappings.size(); ++mappingIndex) {
    ...
    resultFile << "\nNormalization A = " << fitFunction->GetParameter(0)
               ...
               << "\nChi2 = " << chi2 << " ndf " << ndf << " Ch2/ndf " << (double)chi2/ndf << "\n\n";
