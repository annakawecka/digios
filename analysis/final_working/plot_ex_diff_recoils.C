...
void plot_ex_diff_recoils() {
  ...
  TLegend *leg = new TLegend(0.55, 0.7, 0.92, 0.898);
  leg->AddEntry(hist1, "^{17}F(d,p) with ^{18}F recoils", "l");
  leg->AddEntry(hist2_shifted, "^{17}O(d,p) with ^{18}O recoils", "l");
  leg->AddEntry(hist3, "^{17}F(d,p) with ^{17}O recoils", "l");
  //leg->AddEntry(hist4, "^{17}F(d,p) with ^{14}N recoils x 7", "l");
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  ...
  hist2_shifted->GetYaxis()->SetRangeUser(0, 1800);
  ...
  canvas->SaveAs("ex_18O_18F_17O_scaled_2.png");
  canvas->SaveAs("ex_18O_18F_17O_scaled_2.pdf");
}
