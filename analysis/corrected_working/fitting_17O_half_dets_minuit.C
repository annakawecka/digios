...
double modelFunction(double x, const double *par) {
    ...
    for (int i = 0; i < gNpeaks; ++i) {
        ...
        // compute sigma using peak position (linear with x)
        double sigma = sigma0 + sigma1 * (pos - pos0); // or sigma0 + sigma1*pos if you prefer
        // Ensure sigma stays in a sane positive interval (soft clip)
