...
void ExtractPeakInfo_17F() {
  ...
  std::ifstream infile("plots_17F/fitting/fit_parameters_17F.txt");
  if (!infile.is_open()) {
    std::cerr << "Failed to open file\n";
    return;
  }

  std::ofstream outfile("plots_17F/fitting/peak_summary_17F.txt");
  if (!outfile.is_open()) {
  ...
  std::vector<std::tuple<std::string, int, double, double, double, double, double, double, int>> peaks; // hist, peak#, pos, pos_err, int, int_err, matched_true_peak_index
  ...
  while (std::getline(infile, line)) {
    ...
    if (line.find("Peak") != std::string::npos && line.find("Sigma:") != std::string::npos) {
      size_t pos_colon = line.find(":");
      size_t paren_open = line.find("(");
      size_t paren_close = line.find(")");
      sigma = std::stod(line.substr(pos_colon + 1, paren_open - pos_colon - 1));
      sigma_err = std::stod(line.substr(paren_open + 1, paren_close - paren_open - 1));
	...
	if (diff < min_diff) {
      ...
      peaks.emplace_back(current_hist, ++peakNum, pos, pos_err, sigma, sigma_err, integral, integral_err, best_match);
    }
  }
    ...
    * stream << std::string(91, '-') << "\n";
    ...
    for (const auto& peak : peaks) {
      * stream << std::setw(15) << std::get<0>(peak)
	       ...
	       << std::setw(18) << true_peaks[std::get<8>(peak)]
	       << std::setw(12) << std::get<2>(peak)
	       << std::setw(12) << std::get<3>(peak)
	       << std::setw(12) << std::get<2>(peak) - true_peaks[std::get<8>(peak)]
	       << std::setw(10) << std::get<4>(peak)
	       << std::setw(10) << std::get<5>(peak)
	       << std::setw(14) << std::get<6>(peak)
	       << std::setw(12) << std::get<7>(peak) << "\n";
    }
    *stream << "\n";
  }
}
